                    ...
                    subsubMenu: [
                        {
                            ...
                            svgData: <svg className={`${styles.circle} ${styles.svgTV}`} xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512"><path d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512z"/></svg>,
                            elements : [
    ...
    return (
        <section className={styles.features}>
            ...
            <form className={styles.features_form}>
                <input aria-label="Que recherchez-vous ?" type="text" placeholder='Quel service recherchez-vous ?' />
                <figure>
            ...
            <div className={styles.features_all}>
                <div className={styles.features_choice}>
                    {datas.map(link => (
                        <h4 key={link.id} onClick={() => toggleTab(link.id)} className={`${openTab === link.id ? styles.feature_active : ''} ${styles.ripple_effect}`}>
                        {link.title}
                      </h4>
                    ))}
                </div>
                    ...
                    <div key={link.id} className={openTab === link.id ? styles.feature_open : ""}>
                        <div className={styles.features_cards}>
                            {link.subMenus.map(subMenu => (
                                <div key={subMenu.id}>
                                    <div onClick={() => handleDivClick(subMenu.id)} className={styles.features_card} style={{ backgroundColor: isActive === subMenu.id ? '#F2F4F6' : 'transparent', height: isActive === subMenu.id ? '100%' : 'auto', borderRadius: isActive === subMenu.id ? '8px 8px 0 0' : '8px' }}>
                                        <div>
                                            {subMenu.svgData}
                                            <h5>{subMenu.title}</h5>
                                        </div>
                                ...
                                {menuOpen === subMenu.id && (
                                    <div className={styles.features_cardContent}>
                                        ...
                                        <div key={sub.id}>
                                            ...
                                            <div>
                                                {sub.order ? <button>{sub.order}</button> : "" }
                ...
                <div>
                    <div className={styles.features_all}>
                        <div className={styles.features_choice}>
                            {datas.map(link => (
                                <div key={link.id}>
                                    <div onClick={() => toggleTabSmall(link.id)}
                                    ...
                                    <div className={styles.features_type}>
                                        <div className={openTabSmall === link.id ? styles.feature_open : ""}>
                                            <div className={styles.features_cards}>
                                                {link.subMenus.map(subMenu => (
                                                    <div key={subMenu.id}>
                                                        <div onClick={() => handleDivClick(subMenu.id)} className={`${styles.features_card} `}>
                                                            <div>
                                                                {subMenu.svgData}
                                                                <h5>{subMenu.title}</h5>
                                                            </div>
